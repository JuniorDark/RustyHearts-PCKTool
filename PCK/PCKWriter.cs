...
namespace PCKTool.PCK
{
    public static class PCKWriter
    {
        ...
        public delegate void GetFilesInDropsDelegate(string path, bool isComplete);
        public static List<string> GetFilesInDrops(string dir, GetFilesInDropsDelegate gfDelegate)
        {
        ...
        public static void Packing(string rootDir, List<string> listFile, List<PCKFile> listPckFile, bool replace, PackingDelegate pDelegate)
        {
            ...
            for (int i = 0; i < 10; i++)
            {
                string pathPCK = Path.Combine(Directory.GetCurrentDirectory(), string.Format("{0:000}.pck", i));
            ...
            string pathF00X = Path.Combine(Directory.GetCurrentDirectory(), "f00x.dat");
            if (File.Exists(pathF00X))
            {
                byte[] buffer = File.ReadAllBytes(pathF00X);
            ...
            foreach (KeyValuePair<string, string> file in dicInputFile)
            {
                ...
                FileStream fileStream = File.OpenRead(file.Value);
        ...
        public static void WriteF00XDAT(SortedDictionary<string, PCKFile> dicPckFile)
        {
            ...
            if (ZLibDll.Compress(bufferF00XDAT, bufferF00XDAT.Length, bufferCompress, ref numCompress) == 0)
            {
                ...
                string pathF00XDAT = Path.Combine(Directory.GetCurrentDirectory(), "f00X.dat");
